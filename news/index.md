...
- BUG-FIX - `perc_meandiff` calculated the degree of freedom incorrectly
  for equal variance differences (see
  [\#106](https://github.com/dcr-unibe-ch/presize/issues/106))
...
- notes in helpfile for `prec_mean` and `prec_meandiff` that the former
  is suitable for paired mean differences. Comparable changes in the
  shiny app.
...
- minor clarification for `prec_icc` regarding assumption of no
  ...
  [\#97](https://github.com/dcr-unibe-ch/presize/issues/97))
- address warnings with prec_prop (see
  [\#94](https://github.com/dcr-unibe-ch/presize/issues/94))
